...
namespace DeBOTCBot
{
    ...
    public class ServerInfo(DiscordGuild guild)
    {
        ...
        //}
        //public class GrimBuilder(ulong messageID, string script)
        //{
        //    public ulong message = messageID;
        //    public int grimStageID = 0;
        //    public string script = script;
        //    public int players = 5;
        //    public List<Token> currentTokens = [];
        //    public (int, int, int, int) targetTokens = (0, 0, 0, 0);
        ...
        //public Dictionary<ulong, GrimBuilder> currentGrimBuilders = [];
    ...
    public static class AppCommands
    {
        ...
        public static async Task MessageCommand(SlashCommandContext context)
        {
            ServerInfo info = DeBOTCBot.activeServers[1396921797160472576];
            ...
            {
                info.Log(exception);
                builder = new DiscordMessageBuilder().WithContent("Something went wrong!");
            }
            await context.RespondRegistered(info, builder);
        }
        public static DiscordMessageBuilder TestMessage()
        {
            DiscordMessageBuilder builder = new();
            builder.EnableV2Components();
            DiscordSectionComponent section = new(new DiscordTextDisplayComponent("Button?"), new DiscordButtonComponent(DiscordButtonStyle.Primary, "deB_BOTCTemp", "Button!"));
            DiscordSectionComponent section2 = new(new DiscordTextDisplayComponent("Button?"), new DiscordButtonComponent(DiscordButtonStyle.Primary, "deB_BOTCTemp2", "Button!"));
            builder.AddContainerComponent(new DiscordContainerComponent([section, section2]));
            return builder;
        }
    }
    public static class BOTCCommands
    {
        ...
        public static class TokenCommands
        {
            public class TokensAutoComplete : IAutoCompleteProvider
            {
                public async ValueTask<IEnumerable<DiscordAutoCompleteChoice>> AutoCompleteAsync(AutoCompleteContext context)
                {
                    ...
                    {
                        DeBOTCBot.activeServers[context.Guild.Id].Log(exception);
                    }
                    return [];
                }
            }
        ...
        public static class ScriptCommands
        {
            ...
            //[Command("build")]
            //[Description("Start building a grimoire from a specified script")]
            //public static async Task BOTCScriptBuildCommand(SlashCommandContext context, [Description("The script to build a grimoire from")][SlashAutoCompleteProvider<ScriptsAutoComplete>] string script)
            //{
            //    ServerInfo info = DeBOTCBot.activeServers[context.Guild.Id];
            //    info.Log($"Building a grimoire from script: \"{script}\"");
            //    await context.DeferResponseAsync(true);
            //    string response = $"Something went wrong while beginning a grimoire build with script: \"{script}\"!";
            //    DiscordMessageBuilder builder = null;
            //    try
            //    {
            //        if (info.botcGame.ScriptExists(script, out string correctedScript))
            //        {
            //            ulong id = (await context.GetResponseAsync()).Id;
            //            ServerInfo.GrimBuilder newGrim = new(id, script);
            //            info.currentGrimBuilders.Add(id, newGrim);
            //            builder = await DeBOTCBot.UpdateBuildGrimoireMessage(info, newGrim);
            //        }
            //        else
            //        {
            //            response = $"Script with name: \"{script}\" does not exist!";
    ...
    public class DeBOTCBot
    {
        ...
        public static async Task Main()
        {
            ...
            //.HandleGuildMemberUpdated(RoleUpdated)
        ...
        public static async Task Interaction(DiscordClient client, ComponentInteractionCreatedEventArgs args)
        {
            ...
            //            default:
            //                {
            //                    info.Log($"Invalid interaction: \"{args.Id}\"");
            //                    break;
            //                }
            //        }
            //    }
            ...
            //    {
            //        int buttonID = int.Parse(args.Id.Replace("deB_BOTCPlayerVote", ""));
            ...
            //        }
            //    }
            //    else if (args.Id.StartsWith("deB_BOTCGrimBuilder"))
            //    {
            //        ServerInfo.GrimBuilder grimBuilder = info.currentGrimBuilders[args.Interaction.Message.Id];
            //        string grimBuilderID = args.Id.Replace("deB_BOTCGrimBuilder", "");
            //        switch (grimBuilderID)
            //        {
            //            case "Cancel":
            //                {
            //                    info.currentGrimBuilders.Remove(grimBuilder.message);
            //                    await args.Interaction.CreateResponseAsync(DiscordInteractionResponseType.UpdateMessage, new DiscordInteractionResponseBuilder().WithContent("Cancelled grimoire build!"));
            //                    break;
            //                }
            //            case "deB_BOTCGrimBuilderPlayers":
            //                {
            //                    grimBuilder.grimStageID = 1;
            //                    await args.Interaction.CreateResponseAsync(DiscordInteractionResponseType.UpdateMessage, new DiscordInteractionResponseBuilder(await UpdateBuildGrimoireMessage(info, grimBuilder)));
            //                    break;
            //                }
            //        }
            //    }
        ...
        //    info.currentGrimBuilders.Remove(id);
        //    try
        //    {
        ...
        //    try
        //    {
        //        List<ulong> messageIDs = [..info.messages.Keys];
        //        for (int i = 0; i < messageIDs.Count; i++)
        //        {
        //            ulong messageID = messageIDs[i];
        //            if (info.messages[messageID].ChannelId == channelID)
        //            {
        //                info.messages.Remove(messageID);
        //            }
        //        }
        //        if (info.channels.Remove(channelID))
        //        {
        //            await EndGame(info, [..info.playerDictionary.Keys]);
        //        }
        //    }
        ...
        //    return builder;
        //}
        //public static async Task<DiscordMessageBuilder> UpdateBuildGrimoireMessage(ServerInfo info, ServerInfo.GrimBuilder grim)
        //{
        ...
        //    DiscordComponentType[] containerTypes = [DiscordComponentType.ActionRow, DiscordComponentType.TextDisplay, DiscordComponentType.MediaGallery, DiscordComponentType.Separator, DiscordComponentType.File,  DiscordComponentType.Section];
        //    DiscordMessageBuilder builder = new DiscordMessageBuilder().EnableV2Components();
        //    DiscordContainerComponent container = null;
        //    DiscordButtonComponent cancelButton = new(DiscordButtonStyle.Danger, "deB_BOTCGrimBuilderCancel", "Cancel");
        //    int[] actionRowCounts;
        //    switch (grim.grimStageID)
        //    {
        //        case 0:
        //            {
        //                List<DiscordComponent> containerComponents = [];
        //                actionRowCounts = [1, 1];
        //                List<List<DiscordComponent>> actionRows = [];
        //                for (int i = 0; i < actionRowCounts.Length; i++)
        //                {
        //                    actionRows.Add(new(actionRowCounts[i]));
        //                }
        //                containerComponents.Add(new DiscordTextDisplayComponent("Pick a number of players!"));
        //                List<DiscordSelectComponentOption> options = [];
        //                for (int i = 5; i < 16; i++)
        //                {
        //                    string indexString = i.ToString();
        //                    options.Add(new DiscordSelectComponentOption(indexString, indexString, isDefault: i == 5));
        //                }
        //                containerComponents.Add(new DiscordSelectComponent("deB_BOTCGrimBuilderPlayers", "Player Count", options));
        //                containerComponents.Add(cancelButton);
        //                int j = 1;
        //                int k = 0;
        //                for (int i = 0; i < containerComponents.Count; i++)
        //                {
        //                    DiscordComponent component = containerComponents[i];
        //                    if (containerTypes.Contains(component.Type) || actionRowCounts == null || k >= actionRowCounts.Length)
        //                    {
        //                        continue;
        //                    }
        //                    actionRows[k].Add(component);
        //                    containerComponents.RemoveAt(i);
        //                    i--;
        //                    j++;
        //                    if (j > actionRowCounts[k])
        //                    {
        //                        j = 1;
        //                        k++;
        //                    }
        ...
        //                for (int i = 0; i < actionRows.Count; i++)
        //                {
        //                    containerComponents.Add(new DiscordActionRowComponent(actionRows[i]));
        //                }
        //                container = new DiscordContainerComponent(containerComponents, color: DiscordColor.Violet);
        //                for (int i = 0; i < container.Components.Count; i++)
        //                {
        //                    DiscordComponent component = container.Components[i];
        //                    info.Log($"{component.Type}");
        //                    if (component.Type == DiscordComponentType.ActionRow)
        //                    {
        //                        for (int l = 0; l < (component as DiscordActionRowComponent).Components.Count; l++)
        //                        {
        //                            DiscordComponent component1 = (component as DiscordActionRowComponent).Components[l];
        //                            info.Log($"{component1.Type}");
        //                        }
        //                    }
        //                }
        //                break;
        ...
        //            }
        //    }
        //    return builder.AddContainerComponent(container);
        //}
